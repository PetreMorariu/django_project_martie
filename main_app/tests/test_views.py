...
def test_add_book_view(client_logged_in, user, book, books):
    ...
    assert response.status_code == 200
    assert "Book added succesfully!" in response.content.decode()

def test_book_detail(client_logged_in,user):
    url = "/books/add/"
    ...
    response = client_logged_in.post(url, book_dict)
    assert response.status_code == 200
    book = Book.objects.get(title=book_dict['title'])

    assert book.detail.summary == "Book 3 Tes"
    assert book.detail.published_date.day == date.today().day
    assert len(book.detail.isbn) == 13
    id_detail_book = book.detail.id
    book.delete()

    #catch the exception and verify it
    #assert BookDetail.objects.get(id=id_detail_book) is None
