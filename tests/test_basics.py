import pytest
from tests.to_test import (extract_odd_numbers,
...
def test_dog_class():
    #testing from_string method. should receive .from_string("Shadow-4-3")
    dog1 = Dog.from_string("Shadow-4-2")
    assert isinstance(dog1, Dog)
    assert dog1.name == "Shadow"
    assert dog1.legs == 4
    assert dog1.age == 2
    assert dog1.weight == '4kg'
...
def test_is_adult():
    dog1 = Dog.from_string("Shadow-4-2")
    dog2 = Dog.from_string("Shadow-4-5")
    dog3 = Dog.from_string("Shadow-4-4")
    assert Dog.is_adult(dog1.age) == False
    assert Dog.is_adult(dog2.age)
    assert Dog.is_adult(dog3.age) == False
